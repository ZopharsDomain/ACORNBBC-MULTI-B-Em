...
void savecmos()
{
        ...
        fwrite(cmos[0],64,1,f);
        fclose(f);
        f=fopen("cmosc.bin","wb");
        fwrite(cmos[1],64,1,f);
        fclose(f);
}
...
//                set_leds(temp);
        ...
        if (model>=7)
        {
                ...
                cmosold=IC32&4;
                if (cmosstrobe && cmosena && !cmosrw && cmosaddr>0xB) cmos[model-7][cmosaddr]=sdbval; //printf("CMOS write %02X %02X\n",cmosaddr,sdbval);
                if (cmosena && cmosrw) { sysvia.ora=cmos[model-7][cmosaddr]; /*printf("CMOS read %02X %02X\n",cmosaddr,sysvia.ora);*/ }
        }
}
...
void writedatabus(unsigned char val)
{
        ...
        if (!(IC32&1))
           soundwrite(val);
                if (model>=7 && cmosstrobe && cmosena && !cmosrw && cmosaddr>0xB) cmos[model-7][cmosaddr]=sdbval;
}
...
//                printf("Port A clear\n");
                ...
                {
                        sysvia.ora=cmos[model-7][cmosaddr];
...
unsigned char codeconvert[128]=
{
        ...
        112,121,123,42,54,29,0,42, //96
        54,29,29,56,70,69,58,0,    //104
        0,0,58,0,0,0,0,0,          //112
        0,0,0,0,0,0,58,0,          //128
};
